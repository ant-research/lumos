...
class LumosT2IBlock(nn.Module):
    """
    A LumosT2I block with adaptive layer norm (adaLN-zero) conditioning.
    """
...
#############################################################################
#                    Core Lumos Text-to-Image Model                         #
#############################################################################
@MODELS.register_module()
class LumosT2I(nn.Module):
    """
    ...
    def __init__(self, input_size=32, patch_size=2, in_channels=4, hidden_size=1152, depth=28, num_heads=16, mlp_ratio=4.0, class_dropout_prob=0.1, pred_sigma=True, drop_path: float = 0., window_size=0, window_block_indexes=None, use_rel_pos=False, caption_channels=4096, lewei_scale=1.0, config=None, model_max_length=120, **kwargs):
        ...
        self.blocks = nn.ModuleList([
            LumosT2IBlock(hidden_size, num_heads, mlp_ratio=mlp_ratio, drop_path=drop_path[i],
    ...
    def forward(self, x, timestep, y, mask=None, data_info=None, **kwargs):
        """
        Forward pass of Lumos-T2I.
        x: (N, C, H, W) tensor of spatial inputs (images or latent representations of images)
        t: (N,) tensor of diffusion timesteps
        y: (N, 1, 120, C) tensor of caption embeddings
        """
        x = x.to(self.dtype)
    ...
    def initialize_weights(self):
        ...
        # Zero-out adaLN modulation layers in LumosT2I blocks:
        for block in self.blocks:
...
#################################################################################
@MODELS.register_module()
def LumosT2I_XL_2(**kwargs):
    return LumosT2I(depth=28, hidden_size=1152, patch_size=2, num_heads=16, **kwargs)
