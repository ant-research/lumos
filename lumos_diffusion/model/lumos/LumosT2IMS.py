...
from lumos_diffusion.model.builder import MODELS
from lumos_diffusion.model.utils import auto_grad_checkpoint, to_2tuple
from lumos_diffusion.model.lumos.Lumos_blocks import modulate, CaptionEmbedder, WindowAttention, MultiHeadCrossAttention, T2IFinalLayer, TimestepEmbedder, SizeEmbedder
from lumos_diffusion.model.lumos.LumosT2I import LumosT2I, get_2d_sincos_pos_embed
...
class LumosT2IMSBlock(nn.Module):
    """
    A LumosT2IMS block with adaptive layer norm zero (adaLN-Zero) conditioning.
    """
...
######################################################################################################
#                             Core Lumos Text-to-Image Mluti-Scale Model                             #
######################################################################################################
@MODELS.register_module()
class LumosT2IMS(LumosT2I):
    """
    ...
    def __init__(self, input_size=32, patch_size=2, in_channels=4, hidden_size=1152, depth=28, num_heads=16, mlp_ratio=4.0, class_dropout_prob=0.1, learn_sigma=True, pred_sigma=True, drop_path: float = 0., window_size=0, window_block_indexes=None, use_rel_pos=False, caption_channels=4096, lewei_scale=1., config=None, model_max_length=120, **kwargs):
        ...
        self.blocks = nn.ModuleList([
            LumosT2IMSBlock(hidden_size, num_heads, mlp_ratio=mlp_ratio, drop_path=drop_path[i],
    ...
    def forward(self, x, timestep, y, mask=None, data_info=None, **kwargs):
        ...
        for block in self.blocks:
            x = auto_grad_checkpoint(block, x, y, t, y_lens, **kwargs)  # (N, T, D) #support grad checkpoint
    ...
    def initialize(self):
        ...
        # Zero-out adaLN modulation layers in LumosT2IMS blocks:
        for block in self.blocks:
...
#########################################################################################
@MODELS.register_module()
def LumosT2IMS_XL_2(**kwargs):
    return LumosT2IMS(depth=28, hidden_size=1152, patch_size=2, num_heads=16, **kwargs)
